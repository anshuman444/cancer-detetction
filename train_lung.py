...
# Configuration
DATASET_PATH = 'datasets/lung'
MODEL_SAVE_PATH = 'models/lung_cancer_model.h5'
HISTORY_SAVE_PATH = 'json_files/lung cancer/training_history.json'
IMG_SIZE = (224, 224)
BATCH_SIZE = 32
EPOCHS = 5 # Small number for demonstration/quick update

def train():
    # Check if dataset has enough data
    classes = ['aca', 'scc', 'benign']
    for c in classes:
    ...
    model.compile(optimizer='adam', 
                  loss='binary_crossentropy' if train_generator.num_classes == 2 else 'categorical_crossentropy', 
                  metrics=['accuracy'])
